...
def generate_pdf(name,end_date, pr_days):
    ...
    #draw a graph 
    if not os.path.exists('outputs/'+name+'/charts/DCvsAC'):
        os.makedirs('outputs/'+name+'/charts/DCvsAC', mode=0o777)    
    output_file('outputs/'+name+'/charts/DCvsAC/energy_output'+end_date[:10]+'.html')
    ...
    p.y_range.start = 0
    save(p)  
    export_png(p, filename='outputs/'+name+'/charts/DCvsAC/energy_output'+end_date[:10]+'.png',webdriver=web_driver)
    ...
    if system_pr: 
        ...
        if not os.path.exists('outputs/'+name+'/charts/PR'):
            os.makedirs('outputs/'+name+'/charts/PR', mode=0o777)
        #create directory for report
        if not os.path.exists('outputs/'+name+'/charts/reports'):
            os.makedirs('outputs/'+name+'/charts/reports', mode=0o777)
        output_file('outputs/'+name+'/charts/PR/pr_'+end_date[:10]+'.html')
        ...
        save(p)
        export_png(p, filename='outputs/'+name+'/charts/PR/pr_'+end_date[:10]+'.png',webdriver=web_driver)
        ...
        pdf.cell(ln=1, h=5.0, w=0, border=0)
        pdf.cell(ln=1, h=5.0, w=0, border=0)
        pdf.image('outputs/'+name+'/charts/DCvsAC/energy_output'+end_date[:10]+'.png', x = None, y = None, w = 150, h = 120, type = '', link = '')
        pdf.cell(ln=1, h=5.0, w=0, border=0)
        pdf.image('outputs/'+name+'/charts/PR/pr_'+end_date[:10]+'.png', x = None, y = None, w = 170, h = 100, type = '', link = '')
        pdf.set_font('arial', 'B', 10)
        pdf_report_location='outputs/'+name+'/charts/reports/Pv_performance'+end_date[:10]+'.pdf'
